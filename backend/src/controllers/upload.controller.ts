...
import { s3Client, S3_BUCKET, S3_PUBLIC_BASE, PRESIGNED_EXPIRY } from '../config/s3.config';
...
function buildPublicUrl(fileKey: string): string {
  if (S3_PUBLIC_BASE) {
    return `${S3_PUBLIC_BASE.replace(/\/$/, '')}/${fileKey}`;
  }
...
export const getPresignedUploadUrl = async (req: Request, res: Response): Promise<void> => {
  const { entityType, entityId, fileName, fileType, category } =
  ...
  const validEntityTypes: UploadEntityType[] = ['vehicle', 'event'];
  ...
  const validCategories: FileCategory[] = ['photo', 'document'];
...
export const listUploads = async (req: Request, res: Response): Promise<void> => {
  const { entityType, entityId, category } = req.query as ListUploadsQuery;
...
export const getDownloadUrl = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const command = new GetObjectCommand({ Bucket: S3_BUCKET, Key: record.fileKey });
