...
exports.getInventorySummary = exports.deletePart = exports.adjustPartQuantity = exports.updatePart = exports.createPart = exports.getPartById = exports.getAllParts = void 0;
const prisma_1 = __importDefault(require("../prisma"));
const part_types_1 = require("../types/part.types");
...
const getAllParts = async (req, res) => {
    try {
        const { category, vehicleId, lowStock, search } = req.query;
        ...
        const parts = await prisma_1.default.part.findMany({
            where,
            include: partInclude,
...
const getPartById = async (req, res) => {
    try {
        const { id } = req.params;
        const part = await prisma_1.default.part.findUnique({ where: { id }, include: partInclude });
        if (!part) {
...
const createPart = async (req, res) => {
    try {
        ...
        if (!part_types_1.VALID_PART_CATEGORIES.includes(data.category)) {
            res.status(400).json({
                success: false,
                error: `Invalid category. Must be one of: ${part_types_1.VALID_PART_CATEGORIES.join(', ')}`,
            });
            return;
        }
        ...
        const part = await prisma_1.default.part.create({
            data: {
                name: data.name,
...
const updatePart = async (req, res) => {
    try {
        const { id } = req.params;
        const data = req.body;
        const existing = await prisma_1.default.part.findUnique({ where: { id } });
        if (!existing) {
            res.status(404).json({ success: false, error: 'Part not found' });
            return;
        }
        if (data.category &&
            !part_types_1.VALID_PART_CATEGORIES.includes(data.category)) {
            res.status(400).json({
                success: false,
                error: `Invalid category. Must be one of: ${part_types_1.VALID_PART_CATEGORIES.join(', ')}`,
            });
        ...
        const part = await prisma_1.default.part.update({
            where: { id },
            data: {
...
// ---------------------------------------------------------------------------
const adjustPartQuantity = async (req, res) => {
    try {
        const { id } = req.params;
...
exports.adjustPartQuantity = adjustPartQuantity;
// ---------------------------------------------------------------------------
// DELETE /api/parts/:id
// ---------------------------------------------------------------------------
const deletePart = async (req, res) => {
    try {
        ...
        await prisma_1.default.part.delete({ where: { id } });
...
const getInventorySummary = async (req, res) => {
    try {
        const parts = await prisma_1.default.part.findMany({ include: partInclude });
